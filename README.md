...
不要重新執行 `schema.sql`，改成貼上 `supabase/migration_002_add_visits.sql` 的內容並執行一次即可。
這份遷移腳本只會新增「西裝租借人次」與「商品購買人次」兩個欄位，**不會刪除或影響你既有的任何資料**，
舊資料的這兩個新欄位會自動補成 0，之後可以到「原始資料總表」點編輯補登回去。
